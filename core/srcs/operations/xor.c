...
/*   Updated: 2019/07/29 19:21:19 by sofchami         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		operation_xor(t_champ *champ, t_proces *proces, t_arg *args)
{
	...
	else if (args[0].value > REG_NUMBER || args[0].value < 0)
	{
		proces->carry = 0;
		return (-1);
	}
	...
	else if (args[1].value > REG_NUMBER || args[1].value < 0)
	{
		proces->carry = 0;
		return (-1);
	}
	...
	if (args[2].value > REG_NUMBER || args[2].value < 0)
	{
		proces->carry = 0;
		return (-1);
	}
